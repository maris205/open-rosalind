...
from .openhands_runtime import cleanup_isolated_containers
from .task_queue import QUEUE_NAME, redis_connection, release_plan_lock


def main() -> int:
    initialize_database()
    cleaned_containers = cleanup_isolated_containers()
    if cleaned_containers:
        print(f"Removed {cleaned_containers} stale isolated OpenHands container(s).")
