...
当前 macOS alpha.4 已落地 stdio JSON-RPC v4：Worker 可以在最多四轮的
...
当前 macOS alpha.4 已将固定 SHA-256 的 arm64/x86_64 独立 CPython 3.11 与
哈希锁定的 Python wheels 纳入 `.app`/`.dmg`。Release 模式只接受签名包内与应用
架构匹配的解释器，缺失时失败关闭，不再搜索 Homebrew、Xcode Python、用户目录或
`PATH`。正式发行仍需 Developer ID、公证、stapling、Intel 真机和干净系统验证。
