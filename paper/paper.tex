...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{rosalind-principle.png}
\caption{The four operational principles of Open-Rosalind. Each principle constrains the LLM's role and is enforced by a specific layer of the architecture (Figure~\ref{fig:arch}).}
...
Open-Rosalind organizes the four principles into a layered architecture (Figure~\ref{fig:arch}) in which each layer enforces a specific reproducibility invariant. We describe the layers from the bottom up.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{claude-open-rosalind.png}
\caption{The Open-Rosalind layered architecture. Atomic tools and MCP-compliant tool interfaces sit at the base; skills compose tools deterministically; the router and agent dispatch single-step queries; and the harness orchestrates bounded multi-step workflows. Each layer carries its own invariant: tools are deterministic and versioned, skills produce typed outputs, the router emits a categorical decision, and the harness enforces a hard step bound.}
\label{fig:arch}
\end{figure}
...
The combined data flow --- query, mode selection, single-step or harness path, skill execution, evidence and trace generation, and final answer --- is shown in Figure~\ref{fig:workflow}.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/workflow.png}
\caption{End-to-end execution flow. The hybrid mode selector dispatches to either the single-step agent path or the bounded harness path. Both paths converge on the skill registry and the underlying tool layer, and both yield a structured evidence-and-trace bundle that is then summarized into the final answer.}
\label{fig:workflow}
\end{figure}
