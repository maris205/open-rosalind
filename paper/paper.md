...
Open-Rosalind shifts the design of bio-agent systems from free-form reasoning to bounded, traceable workflow execution. Our contribution is best read as an **empirical study with an honest external-validity audit**: a layered framework with explicit invariants at every layer, an experimental realization, a small process-aware benchmark with formally defined metrics, and a diagnosis-and-repair cycle that demonstrates both the value and the limits of the approach.

The internal benchmark evidence supports the tool-first principle as a significant accuracy lever (cluster-permutation $p \le 10^{-3}$, CIs above zero on both models). The external hold-out initially revealed severe validity collapse, but after implementing five diagnosed routing/cleaning fixes, the catastrophic failure was substantially mitigated: the full-vs-no_tool comparison moved from significantly negative ($p=0.002$) to statistically indistinguishable ($p=0.72$, CI $[-23.3, +13.3]$). The remaining full-vs-react gap ($\Delta = -16.7$ pp, $p=0.032$) reflects design tradeoffs — the tool-first invariant excludes LLM-based functional inference — rather than patchable bugs.

We do not claim that the framework's invariants are *causally* responsible for downstream accuracy in all settings; we show that they can be enforced end-to-end without sacrificing performance on a real bioinformatics workload when routing is robust, and we document the concrete failure modes that emerge when routing is not. We hope the framework, the benchmark, the hold-out, the repair cycle, and the released traces are useful both as a working system and as a measurement scaffold for the kind of accountability future bio-agent work should be expected to provide.
