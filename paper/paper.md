...
Open-Rosalind shifts the design of bio-agent systems from free-form reasoning to bounded, traceable workflow execution. Our contribution is best read as a **systems-and-resource paper with an honest external-validity audit**: a layered framework with explicit invariants at every layer, a reference implementation, a small process-aware benchmark with formally defined metrics, and a diagnosis-and-repair cycle that demonstrates both the value and the limits of the approach.
